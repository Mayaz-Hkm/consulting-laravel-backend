...
class AppointmentController extends Controller
{
    ...
    //         $deposit = $expert->hourPrice * 0.2;
    //         $paymentResponse = app(PaymentController::class)->createPaymentIntent(new Request(['amount' => $deposit]));

    //         if ($paymentResponse->getData()->status === 0) {
    //             return response()->json(['status' => 0, 'message' => __('Payment setup failed')], 500);
    //         }
    ...
    public function addAppointment($id, Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        $hasOpenAppointments = $isOpen && $expert->appointments()->where('is_open', true)->exists();
